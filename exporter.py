...
def getcorona_information_from_rki(gen:str) -> any:
    ...
    Returns:
        any: json data
    """
    r = requests.get("https://services7.arcgis.com/mOBPykOjAyBO2ZKk/arcgis/rest/services/RKI_Landkreisdaten/FeatureServer/0/query?where=GEN%20%3D%20'{}'&outFields=EWZ_BL,EWZ,cases_per_population,cases,deaths,death_rate,cases7_per_100k,cases7_bl_per_100k,cases7_bl,death7_bl,cases7_lk,death7_lk,cases7_per_100k_txt&returnGeometry=false&outSR=4326&f=json".format(GEN))
    r.raise_for_status()
    return r.json()
...
def process_request(gaugename: Gauge, api_name: str, corona_data = None):
    ...
    Args:
        ...
        corona_data (str(json)), optional): Json Data from RKI. If no data given it will fetch new. Defaults to None.
    """
    if not corona_data:
